...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    // MOCK PAYMENT INTENT
    if (!process.env.STRIPE_API_KEY || process.env.STRIPE_API_KEY.includes('placeholder')) {
      return res.send({
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (paymentIntentId && paymentIntentId.startsWith("mock_payment_intent_id")) {
        // Skip stripe check for mock payments
      } else {
